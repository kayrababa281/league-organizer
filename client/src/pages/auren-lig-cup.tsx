...
export default function AurenLigCup() {
  ...
  const cupMatches = matches?.filter(m => m.tournament === "auren_lig_cup") || [];
  
  // Get top scorers for this cup
  const topScorers = [...(players || [])]
    .filter(p => (p.aurenLigCupGoals || 0) > 0)
    .sort((a, b) => (b.aurenLigCupGoals || 0) - (a.aurenLigCupGoals || 0))
    .slice(0, 10);
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-3xl font-black font-display text-primary">Auren Lig Cup</h1>
              ...
              <TableBody>
                {topScorers.length === 0 ? (
                  ...
                  <TableRow key={player.id}>
                    ...
                    <TableCell className="text-right font-bold">{player.aurenLigCupGoals}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
